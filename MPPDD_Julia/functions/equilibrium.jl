...
function solveQ!(mod::model,ms::sol,ht::hists,mmt::moments)
    ...
    for qiter = 1:maxiterq
        ...
        if fullcommit == false
            ...
            sbad0 = 0
            sbad0T = 0
            for t=1:N_ages, k=1:N_a, εi =1:N_ε, zi =1:N_z
        ...
        # Q0 = deepcopy(Q);
        if mod.fullcommit == false
            for t=1:N_ages, zi = 1:N_z, api = 1:N_a
        ...
        elseif  fixedassetgrid==false && mod.fullcommit==false
            ...
            for t=1:(N_ages-1), zi = 1:N_z, api = 1:N_a
