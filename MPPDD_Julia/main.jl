...
@everywhere begin #: bcast to all workers, split from the 'using' packages b/c Julia is quirky
    ...
    function params_moments(iofilename, vals, j)
        ...
        outvec = reshape([j, mod.r, mod.β, mod.ϕ_1, mod.ϕ_2, mod.λ , mmt.mpc_ac0, mmt.fr_neg_asset, mmt.avg_debt,mmt.avg_s, mmt.extent_delinq,mmt.fr_s1, mmt.avg_income,mmt.avg_income_neg_asset],1,Nparams+Nmoments+1 );
...
function estimate_elasticities(param_grids::OrderedDict{Symbol,Array{Float64,1}}, file::String = string(saveroot,"out.csv"))
    ...
    params_header = reshape(["idx_num","r","beta","phi_1", "phi_2", "borrow_wdg","MCP_a0" ,"fr_neg_asset", "avg_debt", "avg_s","extent_delinq", "fr_s1", "avg_income","avg_income_neg_asset"],1,Nparams+Nmoments+1 );
...
param_grids  = OrderedDict{Symbol,Array{Float64,1}}([
        ...
        (:β, [0.95, 0.975, 0.99, 0.995]),
        (:ϕ_1,[0.01, 0.1, 0.2, 0.3]),
        (:ϕ_2, [1.25, 2.5, 4, 5.5]),
        (:λ , collect(LinRange(0.001, 0.2, 4))) ]);
...
for j = 1:length(Nparams+1)
    ...
    for zi=1:N_z
        ...
        Qassets = zeros(age_retire-1, N_k_neg)
        for ai=1:N_k_neg
            for t=2:age_retire
