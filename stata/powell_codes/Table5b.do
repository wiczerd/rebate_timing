...
keep tpearn rebate lagrebate rebdum lagrebdum pp time spouse month www famsize flex
...
foreach tau of numlist 0.10(.05).95 {
	preserve
	local tau2=`tau'*100
	**exclude households with any hourly workers
	drop if tpearn>0 & flex>0
	sum time
	local mxx=r(max)
	...
	***this is the same method used in Table A9
	***eliminates any variation from time periods in which household is not working by assigning average values of instrument
	foreach var of varlist *rebdum {
		gen temp=`var'*www if tpearn>0 & flex==0
		egen temp2=sum(temp), by(pp)
		gen temp3=www if tpearn>0 & flex==0
		egen temp4=sum(temp3), by(pp)
		gen mean=temp2/temp4
		replace `var'=mean if tpearn==0
		replace `var'=0 if mean==.
		drop temp* mean
	}
	...
	if (`firr'==1) {
		save nonhourly.dta, replace
		local firr=2
	}
	else {
		append using nonhourly
		save nonhourly, replace
	}
	restore
}
